...
`modelIdentity` 在请求开始时固化，读取历史消息时不得按当前设置回填。工具调用不创建独立聊天 bubble。持久化只保留短摘要、受限来源片段、友好错误和允许的技术详情；不得保存完整 arguments、大 observation 或隐藏 `thought_summary`。持久化来源路径只接受工作区相对 POSIX 路径；旧 RAG 返回的工作区内绝对路径必须先严格匹配当前 workspace 前缀、截取后再次校验并转换为相对路径。其他盘符、UNC、绝对路径、URI 风格路径、包含 `..` 的路径以及任意大小写 `.looma` 路径段必须丢弃。
