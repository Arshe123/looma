...
export const normalizeWorkspaceTabs = (tabs: unknown): WorkspaceTab[] => {
  ...
  for (const item of tabs) {
    ...
    } else if (tab.kind === 'system' && ((tab as any).page === 'settings' || (tab as any).page === 'rag-index' || (tab as any).page === 'ai-history' || (tab as any).page === 'agent-diff' || (tab as any).page === 'help')) {
...
export const getTabTitle = (tab: WorkspaceTab) => {
  if (tab.kind === 'system') {
    ...
    if (tab.page === 'ai-history') return '历史对话'
    if (tab.page === 'agent-diff') return '文件对比'
    if (tab.page === 'help') return '帮助文档'
    return '系统页面'
  }
